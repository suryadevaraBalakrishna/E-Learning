...
export default function AddTeam() {
 ...
 let handleDelete = ((id) => {
    if (window.confirm("Are you sure to delete?")) {
      axios.post(import.meta.env.VITE_API_ADMIN_URL + import.meta.env.VITE_API_TEAM_SETTING_DELETE, {
        id: id
      }).then((result) => {
        if (result.data._status == true) {
  ...
  let handleSubmit = (event) => {
    ...
    axios.post(import.meta.env.VITE_API_ADMIN_URL+import.meta.env.VITE_API_TEAM_SETTING_CREATE,formData)
    .then((result)=>{
      if(result.data._status==true){
        toast.success(result.data._message);
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <button className="btn btn-sm btn-warning me-1" >
                              <a href={`/team/edit/${items._id}`} className="text-white">
